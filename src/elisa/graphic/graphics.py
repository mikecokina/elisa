...
from elisa import units, utils
from elisa import units as u
from elisa.graphic import utils as gutils
...
# Threshold for automatically enabling rasterization when data points exceed this count
DATAPOINT_RASTERIZATION_THRESHOLD = 10000


def orbit(**kwargs) -> Figure | None:
...
def equipotential(**kwargs) -> Figure | None:
    """Plot function for descriptor = `equipotential` in function BinarySystem.plot().

    This function plots crossections of
...
def equipotential_single_star(**kwargs) -> Figure | None:
    """Plot function for descriptor = `equipotential` in function SingleSystem.plot().

    Calculates zx plane crossection of equipotential surface.
...
def single_star_mesh(**kwargs) -> Figure | None:
...
def binary_mesh(**kwargs) -> Figure | None:
...
def single_star_surface(**kwargs) -> Figure | None:
    ...
    :**kwargs options**:
        ...
        * **colorbar** * -- bool; colorbar on/off switch
        * **colorbar_separation** * -- float; shifting position of the colorbar from its default position
...
def binary_surface(**kwargs) -> Figure | None:  # noqa: C901, PLR0912, PLR0915
    # TODO: split to smaller functions!  # noqa: FIX002, TD002, TD003
    ...
    if kwargs["components_to_plot"] == "primary":
        plot = ax.plot_trisurf(
            kwargs["points_primary"][:, 0],
            kwargs["points_primary"][:, 1],
            kwargs["points_primary"][:, 2],
        ...
        if kwargs.get("normals", False):
            ax.quiver(
                kwargs["primary_centres"][:, 0],
                kwargs["primary_centres"][:, 1],
                kwargs["primary_centres"][:, 2],
                kwargs["primary_arrows"][:, 0],
                kwargs["primary_arrows"][:, 1],
                kwargs["primary_arrows"][:, 2],
                color="black",
                length=0.05,
            )
        ...
        plot = ax.plot_trisurf(
            kwargs["points_secondary"][:, 0],
            kwargs["points_secondary"][:, 1],
            kwargs["points_secondary"][:, 2],
            triangles=kwargs["secondary_triangles"],
            antialiased=True,
            shade=True,
            color=clr[1],
        )
        ...
        if kwargs.get("normals", False):
            ax.quiver(
                kwargs["secondary_centres"][:, 0],
                kwargs["secondary_centres"][:, 1],
                kwargs["secondary_centres"][:, 2],
                kwargs["secondary_arrows"][:, 0],
                kwargs["secondary_arrows"][:, 1],
        ...
        if not kwargs["separate_colormaps"]:
            points = up.concatenate((
                kwargs["points_primary"],
                kwargs["points_secondary"]),
                axis=0,
            )
            triangles = up.concatenate(
                (
                    kwargs["primary_triangles"],
                    kwargs["secondary_triangles"] + np.shape(kwargs["points_primary"])[0]),
                axis=0,
            )
        ...
        msg = (
            "Invalid value of keyword argument `components_to_plot`. "
...
def set_colorbar_label(
    ...
    """Set colorbar label with physical quantity and units."""
    lbl = {
        "temperature": "T",
...
def single_star_wireframe(**kwargs) -> Figure | None:
...
def binary_surface_anim(**kwargs) -> None:  # noqa: C901, PLR0915
    """Create animation of the orbital motion.
    ...
    if not kwargs["separate_colormaps"]:
        points = [
            [
                up.concatenate((
                    kwargs["points_primary"][ii],
                    kwargs["points_secondary"][ii]),
                    axis=0,
                )
        ...
        if kwargs.get("colormap", False):
            ...
            cmaps = [
                [
                    up.concatenate((
                        kwargs["primary_cmap"][ii],
                        kwargs["secondary_cmap"][ii],
                    ), axis=0,
                    )
                    for ii in range(kwargs["n_frames"])
                ],
        ...
        plot = [ax.plot_trisurf(
            kwargs["points_primary"][0][:, 0],
            kwargs["points_primary"][0][:, 1],
            kwargs["points_primary"][0][:, 2],
            triangles=kwargs["faces_primary"][0],
            antialiased=True,
            shade=False,
            color=clr[0],
        ),
            ax.plot_trisurf(
                kwargs["points_secondary"][0][:, 0],
                kwargs["points_secondary"][0][:, 1],
                kwargs["points_secondary"][0][:, 2],
                triangles=kwargs["faces_secondary"][0],
                antialiased=True,
                shade=False,
                color=clr[1]),
        ]
...
def single_surface_anim(**kwargs) -> None:
    """Create animation of the orbital motion."""
...
def phase_curve(**kwargs) -> Figure | None:
    """Plot light curve normalized by maximum flux or in magnitude scale.

    :param kwargs: dict; keyword arguments for the plot configuration
    :**kwargs options**:
        * **phases** * -- numpy.ndarray; array of phases
        * **fluxes** * -- dict[str, numpy.ndarray]; dictionary of flux arrays by passband/component
        * **unit** * -- str or astropy.units.Unit; 'normalized', or astropy unit
        * **legend** * -- bool; if True, display legend
        * **legend_location** * -- str; matplotlib legend location string
        * **return_figure_instance** * -- bool; if True, return Figure instance; if False, call plt.show()

    :return: Figure | None; matplotlib Figure instance if return_figure_instance is True, otherwise None
    """
    fig = plt.figure(figsize=(8, 6))
    if kwargs["unit"] in ["normalized", "normalised"]:

        max_flux = np.max([kwargs["fluxes"][item] for item in kwargs["fluxes"]])
    else:
        max_flux = 1
    for item in kwargs["fluxes"]:
        plt.plot(kwargs["phases"], kwargs["fluxes"][item] / max_flux, label=item)
        plt.legend()
...
def rv_curve(**kwargs) -> Figure | None:
    """Plot radial velocity curves for binary system components.

    :param kwargs: dict; keyword arguments for the plot configuration
    :**kwargs options**:
        * **phases** * -- numpy.ndarray; array of phases
        * **rvs** * -- dict[str, numpy.ndarray]; dictionary of radial velocity arrays by component
        * **unit** * -- astropy.units.Unit; radial velocity unit
        * **legend** * -- bool; if True, display legend
        * **legend_location** * -- str; matplotlib legend location string
        * **return_figure_instance** * -- bool; if True, return Figure instance; if False, call plt.show()
...
def binary_rv_fit_plot(**kwargs) -> Figure | None:
    """Plot the model and residuals described by fit params or calculated by last run of fitting procedure.

    Creates a two-panel plot with model fits and residuals for radial velocity data of both components.

    :param kwargs: dict; keyword arguments for the plot configuration
    :**kwargs options**:
        * **synth_phases** * -- numpy.ndarray; synthetic phases for the model curve
        * **rv_fit** * -- dict[str, numpy.ndarray]; fitted RV curves for 'primary' and 'secondary' components
        * **x_data** * -- dict[str, numpy.ndarray]; observed phases for each component
        * **y_data** * -- dict[str, numpy.ndarray]; observed radial velocities for each component
        * **y_err** * -- dict[str, numpy.ndarray | None]; errors in radial velocities for each component
        * **residuals** * -- dict[str, numpy.ndarray]; residuals between model and observations for each component
        * **y_unit** * -- str or astropy.units.Unit; unit for radial velocity axis
        * **return_figure_instance** * -- bool; if True, return Figure instance; if False, call plt.show()
    ...
    if kwargs["y_err"]["primary"] is None:
        ax1.scatter(
            kwargs["x_data"]["primary"],
            kwargs["y_data"]["primary"],
            marker="o",
            color="blue",
            s=3,
            label="primary")
    ...
    if kwargs["y_err"]["secondary"] is None:
        ax1.scatter(
            kwargs["x_data"]["secondary"],
            kwargs["y_data"]["secondary"],
            marker="x",
            color="red",
            s=3,
            label="secondary",
...
def binary_lc_fit_plot(**kwargs) -> Figure | None:
    """Plot the model and residuals described by fit params or calculated by last run of fitting procedure.

    Creates a two-panel plot with model fits and residuals for light curve data. Handles multiple passbands
    and rasterization for large datasets.

    :param kwargs: dict; keyword arguments for the plot configuration
    :**kwargs options**:
        * **lcs** * -- dict[str, numpy.ndarray]; synthetic light curves for each passband
        * **synth_phases** * -- numpy.ndarray; synthetic phases for the model curve
        * **x_data** * -- dict[str, numpy.ndarray]; observed phases for each passband
        * **y_data** * -- dict[str, numpy.ndarray]; observed fluxes/magnitudes for each passband
        * **y_err** * -- dict[str, numpy.ndarray | None]; errors in flux/magnitude for each passband
        * **residuals** * -- dict[str, numpy.ndarray]; residuals between model and observations
        * **rasterize** * -- bool | None; if None, automatically enable for datasets with >10000 points
        * **legend** * -- bool; if True, display legend
        * **loc** * -- str; matplotlib legend location string
        * **return_figure_instance** * -- bool; if True, return Figure instance; if False, call plt.show()
    ...
    for fltr, curve in kwargs["lcs"].items():
        if kwargs["rasterize"] is None:
            rasterize = np.shape(kwargs["x_data"][fltr])[0] > DATAPOINT_RASTERIZATION_THRESHOLD
        else:
...
def passband_plot(**kwargs) -> None:
    """Plot the passband transmission curve.

    Displays the wavelength-dependent transmission characteristics of an optical passband.

    :param kwargs: dict; keyword arguments for the plot configuration
    :**kwargs options**:
        * **xs** * -- numpy.ndarray; wavelength array
        * **ys** * -- numpy.ndarray; transmission values (0-1)
        * **x_unit** * -- str or astropy.units.Unit; wavelength unit
        * **y_unit** * -- str or astropy.units.Unit; transmission unit
        * **passband** * -- str; name of the passband
        * **return_figure_instance** * -- bool; if True, return Figure instance; if False, call plt.show()
