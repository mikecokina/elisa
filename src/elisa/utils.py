...
def apply_darkside_filter(system_container, components):
    """
...
def darkside_filter(line_of_sight, normals):
    ...
    if (line_of_sight == np.array([1.0, 0.0, 0.0])).all():
        cosines = calculate_cos_theta_los_x(normals=normals)
    else:
        cosines = calculate_cos_theta(normals=normals, line_of_sight_vector=np.array([1, 0, 0]))
